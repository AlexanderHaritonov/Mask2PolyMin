...
def segments_to_polyline(segments, is_closed: bool, tolerance: float = 1.0) -> np.ndarray:
    ...
    If the lines are nearly parallel, or their intersection lands implausibly far from the anchor, the vertex falls back to the anchor.
...
def _corner(seg_a, seg_b, tolerance: float) -> np.ndarray:
    ...
    Line intersection when it lands near the junction anchor; otherwise the anchor itself (near-parallel lines).
    ...
    if gap > 0:
        ...
        anchor = seg_a.whole_sequence[orphan_indices].astype(np.float64).mean(axis=0)
    else:
