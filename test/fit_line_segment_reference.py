"""Independent reference implementation of the TLS line fit (np.cov + np.linalg.eigh).

Production fits lines via FitterToPointsSequence.fit_range (O(1) from prefix moments);
this module is deliberately kept on a different algorithm and serves as the oracle
those fits are cross-checked against (test_fit_range.py), and as an array-based
fitting utility for tests.

Run directly to eyeball fits on two real mask edges (needs SHOW_PLOTS-like GUI backend):
    python test/fit_line_segment_reference.py
"""
import os
import sys
...
sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))  # repo root, for direct execution
...
# Two real mask edges (dense pixel contour fragments) for smoke tests and eyeballing.
...
if __name__ == '__main__':
    ...
    for edge, title in [(TEST_EDGE1, "Test Edge 1 with Fitted Line"), (TEST_EDGE2, "Test Edge 2 with Fitted Line")]:
        ...
        print(title, "-", len(points), "points")
        print(params)
        visualize_edge_with_fit(points, params, title)
